...
async function runSimulation() {
  ...
  userSocket.on('conversation-ready', (data) => {
    ...
    setTimeout(() => {
      ...
      userSocket.emit('user-message', {
        ...
        guest_id: 'guest_sim',
        message: 'Hola, necesito ayuda con un pedido (simulator)'
      });
    }, 500);
  });
  ...
  agentSocket.on('agent-registered', (data) => {
    ...
    // Wait a moment and then listen for new-user-message
  });
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        agentSocket.emit('agent-message', {
          ...
          session_id: sessionId,
          agent_id: 'agent_sim',
          message: 'Hola, soy un agente simulado. ¿En qué puedo ayudarte?'
        });
      }, 500);
    }, 500);
  });
