...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    // Verificar si ya existen registros de España
    const [existingRecords] = await queryInterface.sequelize.query(
      "SELECT COUNT(*) as count FROM postal_codes WHERE country = 'ES'"
    );
    
    if (existingRecords[0].count > 0) {
      console.log(`⚠️  Ya existen ${existingRecords[0].count} registros de España en la base de datos`);
      ...
      console.log(`   DELETE FROM postal_codes WHERE country = 'ES';`);
      return;
    }
